...
#define APPNAME "TSSW_CameraScannerNative"
...
static int set_text_field_value(fz_context *ctx, pdf_document *doc, pdf_obj *field, const char *text)
{
	...
	__android_log_print(ANDROID_LOG_ERROR, "Tag2", "%s",text);
	...
	fz_try(ctx)
	{
		...
		if (code)
		{
			if (strstr(code, "AFNumber_Format"))
				...
				__android_log_print(ANDROID_LOG_ERROR, "pdf_text_widget_content_type field:", "%c",*code);
