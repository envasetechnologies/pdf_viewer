...
#define JNI_FN(A) Java_com_tssw_newmupdf_ ## A
#define PACKAGENAME "com/tssw/newmupdf"
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_getWidgetNamesInternal)(JNIEnv * env, jobject thiz, int pageNumber)
{
	...
	for (widget = pdf_first_widget(ctx, idoc, (pdf_page *)pc->page); widget; widget = pdf_next_widget(ctx, widget))
	{
		...
		text = pdf_field_name(ctx, idoc, annot->obj);
        (*env)->SetObjectArrayElement(env,arr,count,(*env)->NewStringUTF(env, text));  		
		count ++;
	}
...
JNIEXPORT jstring JNICALL
JNI_FN(MuPDFCore_getFocusedWidgetNameInternal)(JNIEnv * env, jobject thiz)
{
	char *text = "";
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_getFocusedWidgetContentTypeInternal)(JNIEnv * env, jobject thiz)
{
	char *text = "";
	...
	fz_try(ctx)
	{
		...
		if (idoc)
		{
			...
			if (focus)
				//text = pdf_text_widget_text(ctx, idoc, focus);
				result = pdf_text_widget_content_type(ctx, idoc, focus);
		}
	}
	fz_catch(ctx)
	{
	...
	if (idoc && glo->current_path)
	{
		...
		if (tmp)
		{
			...
			if (written)
			{
				...
				//added
				glo->doc = fz_open_document(ctx, glo->current_path);
				alerts_init(glo);
